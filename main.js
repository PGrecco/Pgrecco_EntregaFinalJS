// //Llamada al JSON (array de productos):
...
//         // Ahora 'datos' contiene el array de productos
//         console.log(datos);

//         // Puedes hacer lo que quieras con los datos, como mostrarlos en la página web, etc.
//         // Por ejemplo, puedes recorrer los productos y mostrarlos en la consola:
...
// // Llama a la función para cargar los datos cuando se cargue la página o cuando sea necesario
// cargarDatos();
...
// CODIGO PASADO EN LIMPIO CON COMENTARIOS DESCRIPTIVOS
...
async function cargarDatos() {
    try {
        const respuesta = await fetch('jsonServer/productos.json');
        const datos = await respuesta.json();

        // Llamada a la función para renderizar productos usando los datos del JSON
...
function agregarAlCarrito(producto) {
    ...
    if (!descuentoAplicado && total >= 15000) {
        aplicarDescuento();
    }
...
// Llamada a la función para cargar y renderizar productos cuando se cargue la página
document.addEventListener("DOMContentLoaded", function () {
    cargarDatos();
});
